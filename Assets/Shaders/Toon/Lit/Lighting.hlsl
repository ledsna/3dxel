...
{
    half NdotL = Quantize(4, saturate(dot(normal, lightDir)));
...
half3 LightingSpecular(half3 lightColor, half3 lightDir, half3 normal, half3 viewDir, half4 specular, half smoothness)
{
    ...
    half NdotH = half(Quantize(4, saturate(dot(normal, halfVec))));
    ...
    float3 specularReflection = specular.rgb;
...
{
    half NdotL = Quantize(4, saturate(dot(normalWS, lightDirectionWS)));
    half3 radiance = lightColor * (Quantize(4, lightAttenuation) * NdotL);
    ...
    [branch] if (!specularHighlightsOff)
    {
        ...
        half brdfCoat = kDielectricSpec.r * DirectBRDFSpecular(brdfDataClearCoat, normalWS, lightDirectionWS, viewDirectionWS);
            ...
            half NoV = Quantize(4, saturate(dot(normalWS, viewDirectionWS)));
